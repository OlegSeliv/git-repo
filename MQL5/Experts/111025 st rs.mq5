...
double CalcAutoLot(double stopLossPoints){ if(stopLossPoints<=0) return NormalizeLot(0.01); if(!AccountInfoDouble(ACCOUNT_BALANCE)) return NormalizeLot(0.01); if(!SymbolInfoDouble(g_symbol,SYMBOL_TRADE_TICK_VALUE)){} double balance=AccountInfoDouble(ACCOUNT_BALANCE); double risk=balance*(InpRiskPerTradePct/100.0); double tv=SymbolInfoDouble(g_symbol,SYMBOL_TRADE_TICK_VALUE); double ts=SymbolInfoDouble(g_symbol,SYMBOL_TRADE_TICK_SIZE); double moneyPerLot=(stopLossPoints*g_point/ts)*tv; if(moneyPerLot<=0) return NormalizeLot(InpMinLot); double lots=risk/moneyPerLot; lots=MathMax(lots,InpMinLot); return NormalizeLot(lots); }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result){ if(!InpEnableSounds) return; if(trans.type==TRADE_TRANSACTION_DEAL_ADD){ if(HistoryDealSelect(trans.deal)){ long entry=HistoryDealGetInteger(trans.deal,DEAL_ENTRY); long reason=HistoryDealGetInteger(trans.deal,DEAL_REASON); if(entry==DEAL_ENTRY_OUT || entry==DEAL_ENTRY_OUT_BY || reason==DEAL_REASON_SL || reason==DEAL_REASON_TP){ PlaySound(InpSoundExit); } } } }
